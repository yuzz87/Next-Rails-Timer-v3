...
export default function ThreeBox() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const colors = [
        new THREE.Color(0xff4fd8), // ピンク
        new THREE.Color(0xd65cff), // 紫
        new THREE.Color(0x8b5cff), // 青紫
        new THREE.Color(0x4f8cff), // 青
        new THREE.Color(0xffb84f), // オレンジ
        new THREE.Color(0xff5c8a), // ローズ
      ]

      const speed = 0.45
      const colorIndex = (elapsed * speed) % colors.length

      const currentIndex = Math.floor(colorIndex)
      const nextIndex = (currentIndex + 1) % colors.length
      const mixAmount = colorIndex - currentIndex

      const color = colors[currentIndex].clone().lerp(
        colors[nextIndex],
        mixAmount,
      )
